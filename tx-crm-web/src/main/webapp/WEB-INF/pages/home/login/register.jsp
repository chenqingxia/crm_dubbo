...
<head>
    ...
    <meta  name="description" content="网充值卡寄售平台" />
    <meta name="keywords" content="网充值卡寄售平台" />
    <title>注册 - 网充值卡寄售平台</title>
            ...
            $("#thisFrm").validate({
                ...
                messages: {
                    userName: {
                        ...
                        isUsernameRegex:"账户名必须是字母、中文、数字！",
                        isNickName: "该用户名已被使用"
                    },
                    userPassword:{
                        required: "请设置登录密码",
                    ...
                    companyName: {
                        required: "请输入公司名称",
                        rangelength: "必须是2到32位字符!",
                        isContactNameRegex: "必须是中文，字母！"
                    },
                    contactName: {
                        required: "请输入联系人",
                        rangelength: "必须是2到32位字符!"
...
<div id="logo">
    <a href="/">
        <img src="/images/logo_24.png" alt="网充值卡寄售平台" style="margin-top:30px;margin-left: 30px;"/>
    </a>
</div>
<div class="content w1200">
    <div class="pagesarea">
        <c:if test="${!empty mode && mode eq 'qq'}">
        <h3>欢迎来自<span style="color: red"> 腾讯QQ </span>的用户: <span class="orange">${qq_nickname}</span>，第一次需要您再填写一些必要的资料，以后您将可以通过点击首页的“用QQ帐号登录”图标快速登录！</h3>
        </c:if>
        <form action="/register" name="thisFrm" id="thisFrm" method="post" autocomplete="off">
            ...
            <ul class="form s1000">
                ...
                <li>
                    <c:choose>
                        <c:when test="${!empty mode && mode eq 'qq'}">
                            <label for="userPassword">提现密码：</label>
                        </c:when>
                        <c:otherwise>
	                ...
	                <span class="gray">数字、字母、特殊符组成,6到20位字符!</span>
                </li>
                <li>
                ...
                <li class="label"><a href="javascript:void(0)" class="ShowXieyi">《充值卡寄售平台用户注册协议》</a></li>
            </ul>
        </form>
    </div>
</div> <!-- end regCon -->
...
<div class="xieyiArea tcontent hide">
    <div class="block xieyi">
        <h2 class="tcenter">充值卡寄售平台用户注册协议</h2>
        <h4>1、 服务条款的确认和接纳</h4>
        <p>	充值卡寄售平台网站（以下简称“充值卡寄售平台”）提供的各项基于互联网的服务的所有权和运作权归江苏电子商务有限公司所有。本站提供的服务将完全按照其发布的服务条款和操作规则严格执行。服务使用人（以下称“用户”）在本站发生访问、注册、登陆、查询、提现等网络行为时均默示同意本站全部服务条款，用户必须确认：本协议条款是处理双方权利义务的约定，除非违反国家强制性法律，否则始终有效。使用充值卡寄售平台及其提供的服务，您将受本服务协议和所有有关的政策、条件和准则的约束。如果您不同意本使用条件中的任何一条，您可以不注册充值卡寄售平台。</p>

        <h4>2、服务内容与条款</h4>
        <div class="subP">
            <h6>2.1支付服务</h6>
            <p>1、向用户提供接口文档和技术支持；但必须同充值卡寄售平台经过下一轮的合作谈判和协议签署；</p>
            <p>2、为用户提供各种形式的款项代收服务以及网上交易信息查询和管理功能；</p>
            <p>3、用户必须拥有合法的网站，具备开发能力的技术力量；如果用户网站不合法，则充值卡寄售平台可以随时关闭服务，并且不予结算；</p>
            <h6>2.2提现服务</h6>
            <p>1、用户可于任何工作日的工作时间（9:00-17:00），将账户内T-3日前的“冻结资金”申请提现；充值卡寄售平台会将扣除手续费后的金额结算到用户的银行账户或账户内；</p>
            <p>2、但若因用户原因发生退款失败（如提供银行账户状态异常等），充值卡寄售平台不负任何责任，也不做冲正，且不退还已产生的提现手续费。</p>
            <h6>2.3用户隐私和安全</h6>
            <p>尊重用户个人隐私是本站的一项基本政策。除了用户授权或者国家机关要求外，充值卡寄售平台绝对不会公开、编辑或透露其注册资料及保存在本站中的非公开内容，更不会向任何第三方出售、提供用户资料及相关信息。</p>
            <p>用户一旦注册成功，成为充值卡寄售平台的合法用户，将得到一个密码和用户名。您可随时根据指示改变您的密码。用户需谨慎合理的保存、使用用户名和密码。用户若发现任何非法使用用户帐号或存在安全漏洞的情况，请立即通知本站和向公安机关报案。</p>
        </div>
        <h4>3、所有权声明及责任限制</h4>
        <div class="subP">
            <h6>3.1网络服务内容的所有权</h6>
